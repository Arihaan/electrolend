...
task('verify:tokens', 'Deploy oracles for dev enviroment')
  ...
  .setAction(async ({ verify, all, pool }, localDRE) => {
    ...
    for (const entry of Object.entries(getParamPerNetwork(ReserveAssets, network))) {
      ...
      if (lToken) {
        console.log('\n- Verifying lToken...\n');
        await verifyContract(eContractid.ELToken, await getELToken(lToken), [
          lendingPoolProxy.address,
          tokenAddress,
          treasuryAddress,
          `ElectroLend interest bearing ${token}`,
          `EL${token}`,
          ZERO_ADDRESS,
        ]);
      } else {
        console.error(`Skipping elToken verify for ${token}. Missing address at JSON DB.`);
      }
      if (stableDebt) {
        ...
        await verifyContract(eContractid.StableDebtToken, await getStableDebtToken(stableDebt), [
          lendingPoolProxy.address,
          tokenAddress,
          `ElectroLend stable debt bearing ${token}`,
          `sd${token}`,
          ZERO_ADDRESS,
        ]);
      } else {
      ...
      if (variableDebt) {
        ...
        await verifyContract(
          ...
          [
            lendingPoolProxy.address,
            tokenAddress,
            `ElectroLend variable debt bearing ${token}`,
            `vd${token}`,
            ZERO_ADDRESS,
          ]
        );
