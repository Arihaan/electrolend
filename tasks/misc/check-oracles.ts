...
import ElectroLendConfig from '../../markets/electrolend';
// import { ElectroFallbackOracleFactory } from '../../types/ElectroFallbackOracleFactory';
// import { SimplePriceFeedAdapterFactory } from '../../types/SimplePriceFeedAdapterFactory';
...
task('check:oracle:fallback-oracle', 'check:oracle:fallback-oracle').setAction(
  async ({}, localBRE) => {
    ...
    const prices = ElectroLendConfig.ReserveAssets[networkName as eElectroneumNetwork];
    const keys = Object.keys(prices) as string[];
    const values = Object.values(prices) as string[];

    console.log('⚠️ Fallback Oracle check disabled due to missing factory');
  }
);
...
task(
  ...
  console.log('⚠️ Price Aggregator check disabled due to missing factory');
  // The following code is commented out due to missing imports
