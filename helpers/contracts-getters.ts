import {
  ...
  PriceAggregatorAdapterSimpleImplFactory,
  PriceOracleFactory,
  ReserveLogicFactory,
  ...
  WETH9MockedFactory,
  WETHGatewayFactory,
  SimplePriceFeedFactory,
  SimplePriceFeedAdapterFactory,
...
export const getElectroFallbackOracle = async (address?: tEthereumAddress) =>
  await ElectroFallbackOracleFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.ElectroFallbackOracle}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );

export const getELToken = async (address?: tEthereumAddress) =>
  await ELTokenFactory.connect(
    address || (await getDb().get(`${eContractid.ELToken}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
...
export const getElectroProtocolDataProvider = async (address?: tEthereumAddress) =>
  await ElectroProtocolDataProviderFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.ElectroProtocolDataProvider}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );
...
export const getELTokensAndRatesHelper = async (address?: tEthereumAddress) =>
  await ELTokensAndRatesHelperFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.ELTokensAndRatesHelper}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );
...
export const getWETHMocked = async (address?: tEthereumAddress) =>
  await WETH9MockedFactory.connect(
    address || (await getDb().get(`${eContractid.WETNMocked}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );

export const getMockELToken = async (address?: tEthereumAddress) =>
  await MockELTokenFactory.connect(
    address || (await getDb().get(`${eContractid.MockELToken}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
...
export const getElectroOracle = async (address?: tEthereumAddress) =>
  await ElectroOracleFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.ElectroOracle}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );

export const getSimplePriceFeed = async (address?: tEthereumAddress) =>
  await SimplePriceFeedFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.SimplePriceFeed}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );

export const getSimplePriceFeedAdapter = async (address?: tEthereumAddress) =>
  await SimplePriceFeedAdapterFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.SimplePriceFeedAdapter}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );

export const getPriceAggregatorAdapterSimpleImpl = async (address?: tEthereumAddress) =>
  await PriceAggregatorAdapterSimpleImplFactory.connect(
    address ||
      (
        await getDb()
          .get(`${eContractid.PriceAggregatorAdapterSimpleImpl}.${DRE.network.name}`)
          .value()
      ).address,
    await getFirstSigner()
  );
