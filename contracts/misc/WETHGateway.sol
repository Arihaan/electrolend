...
contract WETHGateway is IWETHGateway, ElectroOwnable {
   ...
   **/
  constructor(address weth, address initialOwner) public ElectroOwnable(initialOwner) {
    WETH = IWETH(weth);
  }
  ...
  function withdrawETH(address lendingPool, uint256 amount, address to) external override {
    IELToken lWETH = IELToken(ILendingPool(lendingPool).getReserveData(address(WETH)).elTokenAddress);
