...
contract UiIncentiveDataProviderV2 is IUiIncentiveDataProviderV2 {
    ...
    returns (AggregatedReserveIncentiveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      try IStableDebtToken(baseData.elTokenAddress).getIncentivesController() returns (
        IElectroIncentivesController lTokenIncentiveController
      ) {
        if (address(lTokenIncentiveController) != address(0)) {
          address lRewardToken = lTokenIncentiveController.REWARD_TOKEN();

          try lTokenIncentiveController.getAssetData(baseData.elTokenAddress) returns (
            ...
            (
              ...
              uint256 lIncentivesLastUpdateTimestamp,
              uint256 lTokenIncentivesIndex
            ) = lTokenIncentiveController.assets(baseData.elTokenAddress);
      ...
      try IStableDebtToken(baseData.stableDebtTokenAddress).getIncentivesController() returns (
        IElectroIncentivesController sdTokenIncentiveController
      ) {
      ...
      try IStableDebtToken(baseData.variableDebtTokenAddress).getIncentivesController() returns (
        IElectroIncentivesController vdTokenIncentiveController
      ) {
    ...
    returns (UserReserveIncentiveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      try IELToken(baseData.elTokenAddress).getIncentivesController() returns (
        IElectroIncentivesController lTokenIncentiveController
      ) {
        if (address(lTokenIncentiveController) != address(0)) {
          address lRewardToken = lTokenIncentiveController.REWARD_TOKEN();
          ...
          lUserIncentiveData.tokenAddress = baseData.elTokenAddress;
