...
contract UiPoolDataProviderV2 is IUiPoolDataProviderV2, Initializable {
  ...
  {
    IElectroOracle oracle = IElectroOracle(provider.getPriceOracle());
    ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      reserveData.elTokenAddress = baseData.elTokenAddress;
    ...
    returns (UserReserveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      userReservesData[i].scaledLTokenBalance = IELToken(baseData.elTokenAddress).scaledBalanceOf(
        user
      );
