...
export const getReserveData = async (
  helper: ElectroProtocolDataProvider,
  ...
  return {
    ...
    address: reserve,
    lTokenAddress: tokenAddresses.elTokenAddress,
    symbol,
    decimals,
...
export const getUserData = async (
  ...
  const [userData, scaledLTokenBalance] = await Promise.all([
    ...
    getELTokenUserData(reserve, user, helper),
  ]);
  ...
  return {
    ...
    currentLTokenBalance: new BigNumber(userData.currentELTokenBalance.toString()),
...
const getELTokenUserData = async (
  reserve: string,
  user: string,
  helpersContract: ElectroProtocolDataProvider
) => {
  const elTokenAddress: string = (await helpersContract.getReserveTokensAddresses(reserve))
    .elTokenAddress;

  const elToken = await getELToken(elTokenAddress);

  const scaledBalance = await elToken.scaledBalanceOf(user);
